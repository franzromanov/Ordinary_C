...
/*
 if(strike_frnk>=strike_plyr)if(strike_frnk-strike_plyr<2&& strike_frnk>1||strike_plyr>1){strike_frnk=0;strike_plyr=0;}
 if(strike_frnk<=strike_plyr)if(strike_plyr-strike_frnk<2&& strike_frnk>1||strike_plyr>1){strike_frnk=0;strike_plyr=0;}
*/
//fight
 fight(&frank,&player);
 ...
 fflush(stdout);
//reaction
 if( hitpoint(&frank_hp,&player_hp,hit(frank,player),&strike_frnk,&strike_plyr,&effect)==-1 )break;
 game_effect(effect);
